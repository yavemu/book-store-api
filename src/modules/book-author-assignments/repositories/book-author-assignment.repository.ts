...
export class BookAuthorAssignmentRepository extends BaseRepository<BookAuthorAssignment> implements IBookAuthorAssignmentRepository {
  ...
  async createAssignment(
    ...
    try {
      ...
      return await this._createEntity(
        createBookAuthorAssignmentDto,
        SUCCESS_MESSAGES.BOOK_AUTHOR_ASSIGNMENTS.CREATED,
      );
    } catch (error) {
  ...
  async getAssignmentProfile(
    ...
    try {
      ...
      return new SuccessResponseDto(
        SUCCESS_MESSAGES.BOOK_AUTHOR_ASSIGNMENTS.FOUND_ONE,
        assignment,
      );
    } catch (error) {
      if (error instanceof HttpException) {
      ...
      throw new HttpException(
        'Failed to get assignment profile',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async updateAssignment(
    ...
    try {
      const assignmentResponse = await this.getAssignmentProfile(assignmentId);
      ...
      return await this._updateEntity(
        ...
        SUCCESS_MESSAGES.BOOK_AUTHOR_ASSIGNMENTS.UPDATED,
      );
    } catch (error) {
      if (error instanceof HttpException) {
        throw error;
  ...
  async deactivateAssignment(
    ...
    try {
      ...
      return await this._softDelete(
        assignmentId,
        SUCCESS_MESSAGES.BOOK_AUTHOR_ASSIGNMENTS.DELETED,
      );
    } catch (error) {
      if (error instanceof HttpException) {
        throw error;
  ...
  async getAllAssignments(
    ...
    try {
      ...
      return new SuccessResponseDto(
        SUCCESS_MESSAGES.BOOK_AUTHOR_ASSIGNMENTS.FOUND_ALL,
        paginatedResult,
      );
    } catch (error) {
      throw new HttpException(
