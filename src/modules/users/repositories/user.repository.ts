...
export class UserRepository extends BaseRepository<User> implements IUserRepository {
  ...
  async registerUser(createUserDto: CreateUserDto): Promise<User> {
    try {
      // Validate uniqueness using inherited methods
  ...
  // ========== VALIDACIONES PRIVADAS ESPECÍFICAS ==========

  private async _validateUniqueConstraints(dto: CreateUserDto): Promise<void> {
    const existingUser = await this._findOne({
      where: [
    ...
    if (existingUser) {
      ...
      if (existingUser.email === dto.email.toLowerCase().trim()) {
        throw new ConflictException('Email already exists');
      }
    }
  }
